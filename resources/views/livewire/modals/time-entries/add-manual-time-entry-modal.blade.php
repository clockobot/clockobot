    ...
    <x-modal-child wire:key="add-new-entry">
        ...
        <div class="w-full flex flex-col space-y-2">
            @if(Auth::user()->is_admin)
                <x-forms.inputs.field wire:model="user_id" name="user_id" type="select" label="{{ __('timer.user') }}">
                    <x-slot name="options">
                        <option disabled> -- {{ __('timer.choose') }} -- </option>
        ...
        <div class="mt-5 flex justify-between">
            ...
            <x-buttons.primary type="button" wire:click="addManualTimeEntry">{{ __('timer.save') }}</x-buttons.primary>
        </div>
